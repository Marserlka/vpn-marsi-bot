...
async def instructions(callback: CallbackQuery) -> None:
    text = (
        ...
        "1. Установите приложение под ваш протокол: AmneziaVPN (AmneziaWG), WireGuard, "
        "или v2rayNG / Happ / Streisand (VLESS) — Happ доступен на iOS, Android, Windows и macOS.\n"
        "2. «📡 Мои подключения» → выберите подключение → «Получить конфиг» — бот пришлёт файл или ссылку.\n"
        "3. Для файла: «Добавить конфигурацию» → «Импортировать из файла». Для ссылки (VLESS): скопируйте её "
        "и в приложении выберите «Добавить подписку по ссылке».\n"
